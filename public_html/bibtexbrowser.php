...
<!--this is version v3f19fb75a86f4287ea79b17344fdb2e7751b8656 -->
...
define('BIBTEXBROWSER','v3f19fb75a86f4287ea79b17344fdb2e7751b8656');
...
//     echo $this->getKey();
  ...
  */
  ...
  //    $bibstr = $this->getIconOrTxt('bib',$iconurl);
  ...
  function getBibLink($iconurl=NULL) {
    ...
    $key = $this->getKey();
    $link = '<a data-toggle="collapse" href="#' . $key . '" aria-expanded="false" aria-controls="' . $key . '">' . $bibstr . '</a>';
    $div = ' <div class="collapse" id="' . $key . '">';
    $div .= '<div class="well"><pre class="purebibtex">' . $this->getText() . '</pre></div></div>';

    // . "" class=\"biburl\" title=\"".$this->getKey()."\" {$href}>$bibstr</a>";
    return $link . $div;
  }
  ...
  function getPreprintLink($iconurl=NULL) {
    $str = 'preprint';

    if( $this->hasField('eprinttype')) {
      $str = $this->getField('eprinttype');
    }

    if ($this->hasField('eprint')) {
        return ' <a'.get_target().' href="'.$this->getField("eprint").'">['.$str.']</a>';
    }

    return '';
  }

  function getSupplementaryLink($iconurl=NULL) {
    $str = 'supplementary';

    if( $this->hasField('supptype')) {
      $str = $this->getField('supptype');
    }

    if ($this->hasField('supplink')) {
        return ' <a'.get_target().' href="'.$this->getField("supplink").'">['.$str.']</a>';
    }
  ...
  function toHTML($wrapped=false) {
      ...
      switch(BIBTEXBROWSER_LAYOUT) { // open row
        ...
        case 'table':
          ...
          break;
        case 'definition':
          $result .= '<dl class="row"><dt class="col-sm-1">';
      ...
      switch(BIBTEXBROWSER_LAYOUT) { // close bibref and open bibitem
        case 'table':
          ...
          break;
        case 'definition':
          $result .= $this->getAbbrv().'</dt><dd class="col-sm-11 offset-sm-3">';
          break;
      }
      }
...
function bib2links_default(&$bibentry) {
  ...
  if (BIBTEXBROWSER_PREPRINT_LINKS) {
    $link = $bibentry->getPreprintLink();
    if ($link != '') { $links[] = $link; };
  }

  if (BIBTEXBROWSER_SUPP_LINKS) {
    $link = $bibentry->getSupplementaryLink();
    if ($link != '') { $links[] = $link; };
  }
...
if (!function_exists('poweredby')) {
  ...
  function poweredby() {
    ...
    $poweredby .= 'Powered by <a href="http://www.monperrus.net/martin/bibtexbrowser/">bibtexbrowser</a><!--v3f19fb75a86f4287ea79b17344fdb2e7751b8656-->';
    $poweredby .= '</div>'."\n";
    return $poweredby;
  }
}
...
class BibDataBase {
  ...
  function tagIndex(){
   ...
   /** Generates and returns an array consisting of all tags with their frequency.
   */
  function tagIndexWithFrequencies(){
    $result = array();
...
<meta name="generator" content="bibtexbrowser v3f19fb75a86f4287ea79b17344fdb2e7751b8656" />
<?php
...
<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">
   <channel>
      ...
      <generator>bibtexbrowser v3f19fb75a86f4287ea79b17344fdb2e7751b8656</generator>
...
class Dispatcher {
  ...
  function diagnosis() {
    ...
    echo "bibtexbrowser version: 3f19fb75a86f4287ea79b17344fdb2e7751b8656\n";
